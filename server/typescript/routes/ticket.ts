...
export class TicketController {
    ...
    async getTicketByStatus(@Param("status") status:string,@Res() response:any){
        try{
            console.log(status)
            const fetchTicketByStatus = await Tickets.find({ status: status }).sort({ updatedAt: -1 });
    ...
    async getTicketByRecipient(@Param("id") id:string,@Res() response:any){
        try{
            const fetchTicketById = await Tickets.find({ recipient: id }).sort({ updatedAt: -1 });
            return response.send({
                message: "Ticket fetch successfully",
    ...
    @Put('/tickets/:id')
    async update (@Body() ticket:ITicket , @Param("id") id:string,@Res() response:any) {
        try {
        const update = {
        ...
        if (fetchTicket.recipient === undefined || fetchTicket.recipient === null) {
            ...
            return response.send({ message: "Ticket updated successfully", ticket: updatedTicket });
        } else {
            console.log("test")
    ...
    async close (@Body() ticket:ITicket , @Param("id") id:string,@Res() response:any) {
        try {
            const filter = { _id: new ObjectId(id) };
